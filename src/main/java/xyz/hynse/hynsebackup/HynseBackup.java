...
public class HynseBackup extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (autoEnabled) {
                    if (autoMode.equalsIgnoreCase("Interval")) {
                        scheduleAutoBackupInterval();
                    } else if (autoMode.equalsIgnoreCase("Time")) {
                        scheduleAutoBackupTime();
                    }
                }
            }
    ...
    private void loadAutoSettings() {
        ...
        if (autoEnabled) {
            autoMode = config.getString("auto.mode");
            if (autoMode.equalsIgnoreCase("Interval")) {
                autoInterval = config.getInt("auto.task.Interval");
            } else if (autoMode.equalsIgnoreCase("Time")) {
                autoTime = config.getString("auto.task.Time");
                autoDay = config.getString("auto.task.Day");
            }
        }
    }

    private void scheduleAutoBackupInterval() {
        new BukkitRunnable() {
            @Override
            public void run() {
                backupWhitelistedWorlds();
            }
        }.runTaskTimerAsynchronously(this, autoInterval, autoInterval);
    }

    private void scheduleAutoBackupTime() {
        new BukkitRunnable() {
            @Override
            public void run() {
                LocalDateTime currentTime = LocalDateTime.now();
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEEE"); // Format to get the day of the week
                String formattedDay = currentTime.format(formatter);
                if (formattedDay.equalsIgnoreCase(autoDay) && currentTime.toLocalTime().toString().equalsIgnoreCase(autoTime)) {
                    backupWhitelistedWorlds();
                }
            }
        }.runTaskTimerAsynchronously(this, 0, 20); // Check every second
    }
